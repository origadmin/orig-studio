...
func (_q *CategoryQuery) QueryUser() *UserQuery {
...
func (_q *CategoryQuery) QueryMedia() *MediaQuery {
...
func (_q *CategoryQuery) QueryArticles() *ArticleQuery {
...
func (_q *CategoryQuery) QueryChannels() *ChannelQuery {
...
func (_q *CategoryQuery) WithUser(opts ...func(*UserQuery)) *CategoryQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) WithMedia(opts ...func(*MediaQuery)) *CategoryQuery {
	query := (&MediaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) WithArticles(opts ...func(*ArticleQuery)) *CategoryQuery {
	query := (&ArticleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) WithChannels(opts ...func(*ChannelQuery)) *CategoryQuery {
	query := (&ChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withArticles; query != nil {
		if err := _q.loadArticles(ctx, query, nodes,
			func(n *Category) { n.Edges.Articles = []*Article{} },
			func(n *Category, e *Article) { n.Edges.Articles = append(n.Edges.Articles, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChannels; query != nil {
		if err := _q.loadChannels(ctx, query, nodes,
			func(n *Category) { n.Edges.Channels = []*Channel{} },
			func(n *Category, e *Channel) { n.Edges.Channels = append(n.Edges.Channels, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *CategoryQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Category, init func(*Category), assign func(*Category, *User)) error {
...
func (_q *CategoryQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Category, init func(*Category), assign func(*Category, *Media)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(media.FieldCategoryID)
	}
	query.Where(predicate.Media(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.MediaColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CategoryQuery) loadArticles(ctx context.Context, query *ArticleQuery, nodes []*Category, init func(*Category), assign func(*Category, *Article)) error {
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(article.FieldCategoryID)
	}
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ArticlesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CategoryQuery) loadChannels(ctx context.Context, query *ChannelQuery, nodes []*Category, init func(*Category), assign func(*Category, *Channel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(channel.FieldCategoryID)
	}
	query.Where(predicate.Channel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ChannelsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CategoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(category.FieldUserID)
		}
