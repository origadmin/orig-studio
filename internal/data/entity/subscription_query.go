...
func (_q *SubscriptionQuery) QuerySubscriber() *UserQuery {
...
func (_q *SubscriptionQuery) WithSubscriber(opts ...func(*UserQuery)) *SubscriptionQuery {
	query := (&UserClient{config: _q.config}).Query()
...
//		GroupBy(subscription.FieldSubscriberID).
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldSubscriberID).
//		Scan(ctx, &v)
...
func (_q *SubscriptionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSubscriber != nil {
			_spec.Node.AddColumnOnce(subscription.FieldSubscriberID)
		}
		if _q.withChannel != nil {
			_spec.Node.AddColumnOnce(subscription.FieldChannelID)
		}
	}
