...
// QueryPlanOrders chains the current query on the "plan_orders" edge.
func (_q *SubscriptionPlanQuery) QueryPlanOrders() *OrderQuery {
...
func (_q *SubscriptionPlanQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *SubscriptionPlanQuery) FirstIDX(ctx context.Context) string {
...
func (_q *SubscriptionPlanQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *SubscriptionPlanQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *SubscriptionPlanQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *SubscriptionPlanQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *SubscriptionPlanQuery) Clone() *SubscriptionPlanQuery {
	...
	return &SubscriptionPlanQuery{
		...
		withPlanOrders:    _q.withPlanOrders.Clone(),
...
// WithPlanOrders tells the query-builder to eager-load the nodes that are connected to
// the "plan_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SubscriptionPlanQuery) WithPlanOrders(opts ...func(*OrderQuery)) *SubscriptionPlanQuery {
	query := (&OrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPlanOrders = query
	return _q
}
...
func (_q *SubscriptionPlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubscriptionPlan, error) {
	...
	if query := _q.withPlanOrders; query != nil {
		if err := _q.loadPlanOrders(ctx, query, nodes,
			func(n *SubscriptionPlan) { n.Edges.PlanOrders = []*Order{} },
			func(n *SubscriptionPlan, e *Order) { n.Edges.PlanOrders = append(n.Edges.PlanOrders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubscriptionPlanQuery) loadSubscriptions(ctx context.Context, query *UserSubscriptionQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *UserSubscription)) error {
	...
	nodeids := make(map[string]*SubscriptionPlan)
	for i := range nodes {
...
func (_q *SubscriptionPlanQuery) loadPlanOrders(ctx context.Context, query *OrderQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*SubscriptionPlan)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(order.FieldPlanID)
	}
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscriptionplan.PlanOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SubscriptionPlanQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(subscriptionplan.Table, subscriptionplan.Columns, sqlgraph.NewFieldSpec(subscriptionplan.FieldID, field.TypeString))
	_spec.From = _q.sql
...
func (_q *SubscriptionPlanQuery) Modify(modifiers ...func(s *sql.Selector)) *SubscriptionPlanSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *SubscriptionPlanSelect) Modify(modifiers ...func(s *sql.Selector)) *SubscriptionPlanSelect {
