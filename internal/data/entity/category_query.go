...
func (_q *CategoryQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Category, init func(*Category), assign func(*Category, *Media)) error {
	...
	for _, n := range neighbors {
		fk := n.category_media
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_media" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_media" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
