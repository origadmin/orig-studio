...
func (_q *UserSubscriptionQuery) QueryOrders() *OrderQuery {
...
func (_q *UserSubscriptionQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *UserSubscriptionQuery) FirstIDX(ctx context.Context) string {
...
func (_q *UserSubscriptionQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *UserSubscriptionQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *UserSubscriptionQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *UserSubscriptionQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *UserSubscriptionQuery) WithOrders(opts ...func(*OrderQuery)) *UserSubscriptionQuery {
	query := (&OrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(usersubscription.FieldStatus).
...
//
//	client.UserSubscription.Query().
//		Select(usersubscription.FieldStatus).
//		Scan(ctx, &v)
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withOrders; query != nil {
		if err := _q.loadOrders(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.Orders = []*Order{} },
			func(n *UserSubscription, e *Order) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserSubscriptionQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *Order)) error {
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.OrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_subscription_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_subscription_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_subscription_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserSubscriptionQuery) Modify(modifiers ...func(s *sql.Selector)) *UserSubscriptionSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *UserSubscriptionSelect) Modify(modifiers ...func(s *sql.Selector)) *UserSubscriptionSelect {
