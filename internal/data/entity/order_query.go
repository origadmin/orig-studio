...
func (_q *OrderQuery) QueryPlan() *SubscriptionPlanQuery {
...
func (_q *OrderQuery) QueryRefunds() *RefundQuery {
...
func (_q *OrderQuery) WithPlan(opts ...func(*SubscriptionPlanQuery)) *OrderQuery {
	query := (&SubscriptionPlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) WithRefunds(opts ...func(*RefundQuery)) *OrderQuery {
	query := (&RefundClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withPlan; query != nil {
		if err := _q.loadPlan(ctx, query, nodes, nil,
			func(n *Order, e *SubscriptionPlan) { n.Edges.Plan = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrderQuery) loadPlan(ctx context.Context, query *SubscriptionPlanQuery, nodes []*Order, init func(*Order), assign func(*Order, *SubscriptionPlan)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Order)
	for i := range nodes {
		fk := nodes[i].PlanID
...
func (_q *OrderQuery) loadRefunds(ctx context.Context, query *RefundQuery, nodes []*Order, init func(*Order), assign func(*Order, *Refund)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(refund.FieldOrderID)
	}
...
func (_q *OrderQuery) Modify(modifiers ...func(s *sql.Selector)) *OrderSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *OrderSelect) Modify(modifiers ...func(s *sql.Selector)) *OrderSelect {
