...
type Notification struct {
	ID        int       `json:"id"`
	Action    string    `json:"action"` // e.g., new_comment, new_like
...
// NotificationRepo defines the storage operations for notifications.
type NotificationRepo interface {
	...
	ListByUser(ctx context.Context, userID int, page, pageSize int) ([]*Notification, int, error)
...
func (uc *NotificationUseCase) ListUserNotifications(ctx context.Context, userID int, page, pageSize int) ([]*Notification, int, error) {
...
func (uc *NotificationUseCase) MarkAsRead(ctx context.Context, id int, userID int) error {
	...
	if n.UserID != userID {
		return nil // Forbidden or just ignore
	}
...
func (uc *NotificationUseCase) GetUnreadCount(ctx context.Context, userID int) (int, error) {
	return uc.repo.GetUnreadCount(ctx, userID)
}

func (uc *NotificationUseCase) DeleteNotification(ctx context.Context, id int, userID int) error {
	n, err := uc.repo.Get(ctx, id)
	if err != nil {
		return err
	}
