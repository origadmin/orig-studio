...
// Register registers all interaction routes
// DEPRECATED: Use resource-specific routes instead
func (h *InteractionHandler) Register(group *gin.RouterGroup) {
	interactions := group.Group("/interactions")
	{
...
func (h *InteractionHandler) registerLikes(g *gin.RouterGroup) {
	...
	{
		// Static routes first
		likes.GET("/status", JWTMiddleware(h.jwtMgr), h.getLikeStatusBatch())

		// Collection routes
		likes.GET("", JWTMiddleware(h.jwtMgr), h.getLikes())
		likes.POST("", JWTMiddleware(h.jwtMgr), h.toggleLike())
	}
}
...
func (h *InteractionHandler) registerFavorites(g *gin.RouterGroup) {
	...
	{
		// Static routes first
		favorites.GET("/check", JWTMiddleware(h.jwtMgr), h.checkFavorite())

		// Collection routes
		favorites.GET("", JWTMiddleware(h.jwtMgr), h.getFavorites())
		favorites.POST("", JWTMiddleware(h.jwtMgr), h.toggleFavorite())
	}
}
...
func (h *InteractionHandler) registerSubscriptions(g *gin.RouterGroup) {
	...
	{
		// Static routes first
		subscriptions.GET("/count", JWTMiddleware(h.jwtMgr), h.getSubscriptionCount())

		// Collection routes
		subscriptions.GET("", JWTMiddleware(h.jwtMgr), h.getSubscriptions())
	}

	followers := g.Group("/followers")
	{
		// Static routes first
		followers.GET("/count", JWTMiddleware(h.jwtMgr), h.getFollowerCount())

		// Collection routes
		followers.GET("", JWTMiddleware(h.jwtMgr), h.getFollowers())
	}
}
...
func (h *InteractionHandler) registerShares(g *gin.RouterGroup) {
	shares := g.Group("/shares")
	{
		...
		shares.POST("", JWTMiddleware(h.jwtMgr), h.createShare())
	}
}
...
func (h *InteractionHandler) getLikeStatusBatch() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"status": map[string]bool{}})
	}
}
