...
func (h *ArticleHandler) Register(r handler.Router) {
	articles := r.Group("/articles")
	{
		...
		articles.POST("", WithJWT(h.jwt, h.createArticle()))
		...
		articles.PUT("/:id", WithJWT(h.jwt, h.updateArticle()))

		// DELETE /articles/:id - Delete article (requires auth)
		articles.DELETE("/:id", WithJWT(h.jwt, h.deleteArticle()))

		// PATCH /articles/:id/state - Update article state (requires auth)
		articles.PATCH("/:id/state", WithJWT(h.jwt, h.updateArticleState()))
	}
}
...
func (h *ArticleHandler) getArticleBySlug() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		slug := c.Param("slug")
		if slug == "" {
			c.JSON(http.StatusBadRequest, gin.H{"code": ErrBadRequest, "message": "slug is required"})
			return
		}
...
func (h *ArticleHandler) updateArticleState() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !validStates[input.State] {
			c.JSON(http.StatusBadRequest, gin.H{"code": ErrBadRequest, "message": "invalid state, must be one of: draft, published, archived"})
			return
		}
