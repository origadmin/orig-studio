...
/**
 * useChannel Hook v3.2
 *
 * 支持多种路由来源:
 * - /c/{token} 或 /channel/{id} → 路径参数方式 (getByToken)
 * - /@{handle}              → 查询参数方式 (username)
...
const useChannel = (options: UseChannelOptions = {}): UseChannelReturn => {
    ...
    const fetchChannel = useCallback(async () => {
        ...
        try {
            ...
            if (token) {
                ...
                // /c/{token} 或 /channel/{id}
                response = await channelApi.getByToken(token);
            } else if (handle) {
                // Mode 2: @username 查询参数方式 (两步方案)
                // /@{handle}
                const queryParams: ChannelQueryParams = {username: handle};
                response = await channelApi.get(queryParams);
            } else if (isAuthenticated && user?.id) {
                // Mode 3: 我的频道
                // /me/channel
                response = await channelApi.getMyChannel();
            } else {
                if (isMounted) {
                    setError('请先登录');
                    return;
            ...
            if (isMounted && response?.code === 0) {
                setChannel(response.data as ChannelDetail);
            }
        } catch (err: any) {
    ...
    const isOwner = useMemo(() => {
        ...
        return String(user.id) === String(channel.user_id) || String(user.id) === String(channel.owner_id);
