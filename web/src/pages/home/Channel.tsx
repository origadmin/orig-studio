    ...
    if (loading) {
        return (
            <div className="space-y-8 animate-pulse">
                <div className="h-48 md:h-80 rounded-2xl bg-gray-200 dark:bg-gray-800"/>
    ...
    return (
        <div className="space-y-8">
            <div className="relative">
                <div className="h-48 md:h-80 rounded-2xl bg-cover bg-center bg-gray-200 dark:bg-gray-800"
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <TabsContent value="videos" className="mt-6">
                    {videos.length === 0 ? (
                        <div className="text-center py-12 text-slate-500 dark:text-gray-400">{t('channel.noVideos')}</div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {videos.map(video => (
                                <Link key={video.id} to={`/watch/${video.short_token}`} className="group">
                                    <div
                                        ...
                                        <div className="p-3">
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-gray-400 mt-2">{formatViews(video.view_count)} {t('common.views')}</p>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                ...
                <TabsContent value="playlists" className="mt-6">
                    <div className="text-center py-12 text-slate-500 dark:text-gray-400">{t('channel.noPlaylists')}</div>
                </TabsContent>
                <TabsContent value="community" className="mt-6">
                    <div className="text-center py-12 text-slate-500 dark:text-gray-400">{t('channel.noCommunity')}</div>
                </TabsContent>
